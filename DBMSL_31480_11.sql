...
// Think that the reduce will mostly sum up the Data
...
var map1 = function () {emit(this.branch, 1);};

var reduce1 = function (key, value) {
    return Array.sum(value);
}

db.Student.mapReduce(map1, reduce1, { out: "result1" });

var map2 = function () {emit(this.division, 1);};

var reduce2 = function(key, value) {
  return Array.sum(value);
};

db.Student.mapReduce(map2, reduce2, { out: "result2" });

var map3 = function () {
    emit(this.name, this.forte);
};
...
db.Student.mapReduce(
    function () {
        if (this.forte.includes('Web Development')) {
            emit(this.name, 1);
        }
    },
    function (k, v) {
...
db.Student.mapReduce(
    function () {
            emit("average", this.forte.length);
    },
    function (k, v)
    {
        return Array.avg(v);
